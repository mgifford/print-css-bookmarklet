...
<a href="javascript:(function()%20%7B%0A%20%20const%20STYLE_ID%20%3D%20%22print-media-highlight-style%22%2C%20TOGGLE_ID%20%3D%20%22print-media-highlight-toggle-button%22%2C%20LEGEND_ID%20%3D%20%22print-media-highlight-legend%22%3B%0A%20%20function%20collectMediaRules()%20%7B%20const%20mediaRules%20%3D%20%7B%20print%3A%20new%20Map()%2C%20screen%3A%20new%20Map()%20%7D%3B%20function%20extract(sheet)%20%7B%20try%20%7B%20if%20(!sheet.cssRules)%20return%3B%20for%20(const%20rule%20of%20sheet.cssRules)%20%7B%20if%20(rule.type%20%3D%3D%3D%20CSSRule.MEDIA_RULE)%20%7B%20const%20mediaType%20%3D%20rule.media.mediaText.includes(%22print%22)%20%3F%20%22print%22%20%3A%20rule.media.mediaText.includes(%22screen%22)%20%3F%20%22screen%22%20%3A%20null%3B%20if%20(!mediaType)%20continue%3B%20for%20(const%20innerRule%20of%20rule.cssRules)%20%7B%20if%20(innerRule.selectorText)%20%7B%20if%20(!mediaRules%5BmediaType%5D.has(innerRule.selectorText))%20mediaRules%5BmediaType%5D.set(innerRule.selectorText%2C%20%5B%5D)%3B%20mediaRules%5BmediaType%5D.get(innerRule.selectorText).push(innerRule)%3B%20%7D%20%7D%20%7D%20else%20if%20(rule.type%20%3D%3D%3D%20CSSRule.IMPORT_RULE)%20%7B%20extract(rule.styleSheet)%3B%20%7D%20%7D%20%7D%20catch%20(e)%20%7B%7D%20%7D%20for%20(const%20sheet%20of%20document.styleSheets)%20extract(sheet)%3B%20return%20mediaRules%3B%20%7D%0A%20%20function%20markScreenHiddenElementsWithMediaOnly(mediaRules)%20%7B%20document.querySelectorAll(%22*%22).forEach(el%20%3D%3E%20%7B%20const%20style%20%3D%20getComputedStyle(el)%3B%20if%20(style.display%20!%3D%3D%20%22none%22)%20return%3B%20let%20matchedMediaRule%20%3D%20false%3B%20for%20(const%20%5Bmedia%2C%20ruleMap%5D%20of%20Object.entries(mediaRules))%20%7B%20for%20(const%20%5Bselector%2C%20rules%5D%20of%20ruleMap.entries())%20%7B%20try%20%7B%20if%20(!el.matches(selector))%20continue%3B%20for%20(const%20rule%20of%20rules)%20%7B%20const%20disp%20%3D%20rule.style.getPropertyValue(%22display%22)%3B%20if%20(disp%20%3D%3D%3D%20%22none%22)%20%7B%20matchedMediaRule%20%3D%20true%3B%20break%3B%20%7D%20%7D%20%7D%20catch%20(e)%20%7B%7D%20if%20(matchedMediaRule)%20break%3B%20%7D%20if%20(matchedMediaRule)%20break%3B%20%7D%20if%20(matchedMediaRule)%20%7B%20el.style.setProperty(%22display%22%2C%20%22block%22%2C%20%22important%22)%3B%20el.classList.add(%22print-media-highlight-screen-hidden%22)%3B%20el.setAttribute(%22data-screen-hidden%22%2C%20%22display%3Anone%20(media)%22)%3B%20el.title%20%3D%20%22display%3Anone%20(media)%22%3B%20%7D%20%7D)%3B%20%7D%0A%20%20function%20clearHighlights()%20%7B%20document.querySelectorAll(%22.print-media-highlight-screen-hidden%22).forEach(el%20%3D%3E%20%7B%20el.classList.remove(%22print-media-highlight-screen-hidden%22)%3B%20el.removeAttribute(%22data-screen-hidden%22)%3B%20el.style.removeProperty(%22display%22)%3B%20%7D)%3B%20%7D%0A%20%20function%20injectStyleAndToggle()%20%7B%20if%20(!document.getElementById(STYLE_ID))%20%7B%20const%20style%20%3D%20document.createElement(%22style%22)%3B%20style.id%20%3D%20STYLE_ID%3B%20style.textContent%20%3D%20%22.print-media-highlight-screen-hidden%20%7B%20outline%3A%203px%20solid%20orange%20!important%3B%20%7D%20%5Bdata-screen-hidden%5D%3A%3Abefore%20%7B%20content%3A%20attr(data-screen-hidden)%3B%20display%3A%20block%3B%20font-size%3A%2010px%3B%20background%3A%20%23ffe%3B%20border%3A%201px%20dashed%20orange%3B%20padding%3A%202px%3B%20margin-bottom%3A%202px%3B%20color%3A%20%23c60%3B%20%7D%20%23print-media-highlight-toggle-button%20%7B%20position%3A%20fixed%3B%20top%3A%2010px%3B%20right%3A%2010px%3B%20z-index%3A%2099999%3B%20background-color%3A%20%23333%3B%20color%3A%20white%3B%20border%3A%20none%3B%20padding%3A%208px%2012px%3B%20border-radius%3A%204px%3B%20cursor%3A%20pointer%3B%20font-family%3A%20sans-serif%3B%20font-size%3A%2014px%3B%20opacity%3A%200.8%3B%20%7D%20%23print-media-highlight-toggle-button%3Ahover%20%7B%20opacity%3A%201%3B%20%7D%22%3B%20document.head.appendChild(style)%3B%20%7D%20if%20(!document.getElementById(TOGGLE_ID))%20%7B%20const%20toggle%20%3D%20document.createElement(%22button%22)%3B%20toggle.id%20%3D%20TOGGLE_ID%3B%20toggle.textContent%20%3D%20%22Toggle%20Screen%20Hidden%20(Media)%22%3B%20document.body.appendChild(toggle)%3B%20%7D%20%7D%0A%20%20injectStyleAndToggle()%3B%0A%20%20let%20state%20%3D%20false%3B%20document.getElementById(TOGGLE_ID).onclick%20%3D%20function()%20%7B%0A%20%20%20%20state%20%3D%20!state%3B%0A%20%20%20%20if%20(state)%20%7B%0A%20%20%20%20%20%20const%20mediaRules%20%3D%20collectMediaRules()%3B%0A%20%20%20%20%20%20markScreenHiddenElementsWithMediaOnly(mediaRules)%3B%0A%20%20%20%20%20%20this.textContent%20%3D%20%22Toggle%20Screen%20Hidden%20(On)%22%3B%0A%20%20%20%20%7D%20else%20%7B%0A%20%20%20%20%20%20clearHighlights()%3B%0A%20%20%20%20%20%20this.textContent%20%3D%20%22Toggle%20Screen%20Hidden%20(Off)%22%3B%0A%20%20%20%20%7D%0A%20%20%7D%3B%0A%7D)();" title="Drag to favorites bar">Print Highlighter 2</a>
